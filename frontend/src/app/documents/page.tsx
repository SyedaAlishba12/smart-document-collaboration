...
import { FileText, Plus, Trash2, Star, Edit2, Copy, FolderInput, Archive, RotateCcw, Info, Eye, Folder } from 'lucide-react';
...
export default function DocumentsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch documents and workspace folders in parallel using the correct backend endpoint route[cite: 7]
      const [docRes, folderRes] = await Promise.all([
        axios.get(`${API_BASE_URL}/api/documents?workspace_id=${workspaceId}`),
        axios.get(`${API_BASE_URL}/api/folders/workspace/${workspaceId}`).catch(() => ({ data: [] }))
      ]);
  ...
  const stripHtmlTags = (html: string) => {
    if (!html) return 'Empty document content...';
    return html.replace(/<[^>]*>?/gm, '');
  };

  // Create Document
  const handleCreateDocument = async () => {
    const activeWorkspaceId = localStorage.getItem('workspace_id') || workspaceId;
    try {
  ...
  // Rename Document
  const handleRenameDocument = async (e: React.MouseEvent, docId: string, currentTitle: string) => {
    e.stopPropagation();
    const newTitle = prompt('Enter new document title:', currentTitle);
    if (!newTitle || newTitle.trim() === '') return;

    try {
      await axios.put(`${API_BASE_URL}/api/documents/${docId}`, { title: newTitle.trim() });
      setDocuments((prev) =>
        prev.map((doc) => (doc.id === docId ? { ...doc, title: newTitle.trim() } : doc))
      );
    } catch (err) {
  ...
  // Duplicate Document
  const handleDuplicateDocument = async (e: React.MouseEvent, doc: DocumentItem) => {
    e.stopPropagation();
    try {
      const response = await axios.post(`${API_BASE_URL}/api/documents`, {
      ...
      const duplicatedDoc = response.data.data || response.data;
  ...
  const executeMoveDocument = async (targetFolderId: string | null) => {
    if (!moveModalDoc) return;
    try {
      await axios.post(`${API_BASE_URL}/api/documents/${moveModalDoc.id}/move`, {
        folder_id: targetFolderId,
      });
      setDocuments((prev) =>
        prev.map((doc) => (doc.id === moveModalDoc.id ? { ...doc, folder_id: targetFolderId || undefined } : doc))
      );
  ...
  const handleArchiveDocument = async (e: React.MouseEvent, docId: string, currentArchived?: boolean) => {
    e.stopPropagation();
    try {
      await axios.put(`${API_BASE_URL}/api/documents/${docId}`, {
        is_archived: !currentArchived,
      });
      setDocuments((prev) =>
        prev.map((doc) => (doc.id === docId ? { ...doc, is_archived: !currentArchived } : doc))
      );
    } catch (err) {
  ...
  // Filter documents based on active tab view mode (Active root documents vs Archived)
  const filteredDocuments = documents.filter((doc) => {
    if (viewMode === 'archived') return doc.is_archived;
    return !doc.is_archived && !doc.folder_id;
  });
            ...
            <div className="flex bg-gray-100 p-1 rounded-xl text-xs font-medium">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition cursor-pointer ${
                  viewMode === 'active' ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-black'
                }`}
              >
                Active Documents
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition cursor-pointer flex items-center gap-1 ${
                  viewMode === 'archived' ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-black'
                }`}
              >
                <Archive className="h-3.5 w-3.5" /> Archive ({documents.filter(d => d.is_archived).length})
              </button>
            </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center min-h-[220px] rounded-2xl border border-dashed border-[var(--border)] bg-[var(--surface)] p-8 text-center text-xs text-[var(--muted)]">
            ...
            <p className="font-medium text-gray-700">
              {viewMode === 'archived' ? 'No archived documents found' : 'No root documents found'}
            </p>
            <p className="text-[11px] text-gray-400 mt-1">
              {viewMode === 'archived' ? 'Documents you archive will appear here.' : 'Get started by creating your very first document above.'}
            </p>
          </div>
        ) : (
                    ...
                    <div className="flex items-center gap-0.5">
                      ...
                      {viewMode === 'active' && (
                        <>
                          ...
                          <button
                            onClick={(e) => { e.stopPropagation(); setMoveModalDoc(doc); }}
                      ...
                      <button
                        onClick={(e) => handleArchiveDocument(e, doc.id, doc.is_archived)}
                        className={`p-1.5 rounded-lg transition cursor-pointer ${
                          doc.is_archived ? 'text-purple-600 bg-purple-50 hover:bg-purple-100' : 'text-gray-300 hover:text-purple-600 hover:bg-purple-50'
                        }`}
                        title={doc.is_archived ? 'Restore from Archive' : 'Archive Document'}
                      >
            ...
            <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl space-y-4 border border-gray-100">
              <div className="flex justify-between items-center border-b pb-3">
                <h3 className="font-bold text-base flex items-center gap-2">
                  <FolderInput className="h-4 w-4 text-indigo-600" /> Move Document to Folder
                </h3>
                <button
                  onClick={() => setMoveModalDoc(null)}
              ...
              <p className="text-xs text-gray-500">
                Select a target folder for <span className="font-semibold text-black">"{moveModalDoc.title}"</span>:
              </p>

              <div className="space-y-2 max-h-60 overflow-y-auto">
                <button
                  onClick={() => executeMoveDocument(null)}
                ...
                >
                  <Folder className="h-4 w-4 text-gray-400" /> Root Directory (No Folder)
                </button>

                {folders.map((folder) => (
                  <button
                    key={folder.id}
                    onClick={() => executeMoveDocument(folder.id)}
                  ...
                  >
                    <Folder className="h-4 w-4 text-indigo-500" /> {folder.name}
                  </button>
                ))}

                {folders.length === 0 && (
                  <p className="text-center text-xs text-gray-400 py-4">No folders created in this workspace yet.</p>
                )}
              </div>
            </div>
          </div>
        ...
        {previewDoc && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl space-y-4 border border-gray-100">
              <div className="flex justify-between items-center border-b pb-3">
                <h3 className="font-bold text-base flex items-center gap-2">
                  <Info className="h-4 w-4 text-black" /> Document Metadata & Preview
                </h3>
                <button
              ...
              <div className="space-y-2 text-xs text-gray-600">
                ...
                <p><strong>Document ID:</strong> {previewDoc.id}</p>
                <p><strong>Workspace ID:</strong> {previewDoc.workspace_id}</p>
                <p><strong>Folder ID:</strong> {previewDoc.folder_id || 'Root (No Folder)'}</p>
                <p><strong>Favorite:</strong> {previewDoc.is_favorite ? 'Yes ⭐' : 'No'}</p>
                <p><strong>Archived:</strong> {previewDoc.is_archived ? 'Yes 📦' : 'No'}</p>
                <div className="pt-2 border-t">
                  <strong className="block mb-1 text-gray-800">Content Preview:</strong>
                  <div className="p-3 bg-gray-50 rounded-xl border max-h-32 overflow-y-auto text-gray-700">
                    {stripHtmlTags(previewDoc.content || '')}
                  </div>
                </div>
              </div>
