  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      try {
        ...
        setDocuments(favoriteDocuments);
      } catch (err) {
        setError(
          getApiErrorMessage(
            err,
            "Unable to load your favorite documents."
          )
        );
      } finally {
        setLoading(false);
  ...
  return (
    <MainLayout>
      <div className="space-y-6">
        <Header
          title="Favorites"
          description="Your favorite documents for quick access."
        />
          ...
          <Card>
            <div className="py-10 text-center">
              <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[var(--surface-muted)]">
                <Star className="h-5 w-5 text-[var(--muted)]" />
              </div>
        ...
        {!loading && !error && documents.length === 0 && (
          <Card>
            <div className="py-12 text-center">
              ...
              <p className="mx-auto mt-2 max-w-md text-sm text-[var(--muted)]">
                Documents you mark as favorites will appear
