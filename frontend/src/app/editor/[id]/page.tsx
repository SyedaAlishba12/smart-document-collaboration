...
export default function EditorPage() {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Image.configure({
        HTMLAttributes: {
          class: 'max-w-full h-auto rounded-xl my-4 border border-gray-200 shadow-sm object-contain',
        },
      }),
      Table.configure({
        resizable: true,
    ...
    editorProps: {
      attributes: {
        class: 'max-w-none focus:outline-none min-h-[550px] text-gray-800 text-base leading-relaxed font-sans p-2',
      },
    },
    onUpdate: ({ editor }) => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (fileId) {
        try {
          const downloadRes = await axios.get(`${apiBase}/api/files/${fileId}/download`);
          if (downloadRes.data?.data?.file_url) {
            imageUrl = downloadRes.data.data.file_url;
          }
        } catch (err) {
          console.error('Could not fetch download URL, falling back to file_url', err);
        }
      }
    ...
    <div className="h-screen bg-[#FBFBFA] flex flex-col overflow-hidden text-[#1A1A1A]">
      ...
      <div className="bg-white border-b border-gray-200 px-8 py-2.5 flex flex-wrap gap-1.5 text-xs font-medium text-gray-700 shadow-sm items-center shrink-0">
        <button
          ...
          className={`px-2.5 py-1 border rounded transition cursor-pointer ${editor.isActive('heading', { level: 1 }) ? 'bg-black text-white' : 'bg-gray-50 hover:bg-gray-100'}`}
        >
          H1
        </button>
        <button
          ...
          className={`px-2.5 py-1 border rounded transition cursor-pointer ${editor.isActive('heading', { level: 2 }) ? 'bg-black text-white' : 'bg-gray-50 hover:bg-gray-100'}`}
        >
          H2
        </button>
        ...
        <button
          ...
          className={`px-2.5 py-1 border rounded transition font-bold cursor-pointer ${editor.isActive('bold') ? 'bg-black text-white' : 'bg-gray-50 hover:bg-gray-100'}`}
        >
          B
        </button>
        <button
          ...
          className={`px-2.5 py-1 border rounded transition italic cursor-pointer ${editor.isActive('italic') ? 'bg-black text-white' : 'bg-gray-50 hover:bg-gray-100'}`}
        >
          I
        </button>
        <button
          ...
          className={`px-2.5 py-1 border rounded transition underline cursor-pointer ${editor.isActive('underline') ? 'bg-black text-white' : 'bg-gray-50 hover:bg-gray-100'}`}
        >
          U
        </button>
        <span className="w-px h-4 bg-gray-300 mx-1"></span>
        <button
          ...
          className={`px-2.5 py-1 border rounded transition cursor-pointer ${editor.isActive('bulletList') ? 'bg-black text-white' : 'bg-gray-50 hover:bg-gray-100'}`}
        >
          Bullet List
        </button>
        <button
          ...
          className={`px-2.5 py-1 border rounded transition cursor-pointer ${editor.isActive('orderedList') ? 'bg-black text-white' : 'bg-gray-50 hover:bg-gray-100'}`}
        >
          Numbered List
        </button>
        <button
          ...
          className={`px-2.5 py-1 border rounded transition cursor-pointer ${editor.isActive('blockquote') ? 'bg-black text-white' : 'bg-gray-50 hover:bg-gray-100'}`}
        >
          Quote
        </button>
        <button
          ...
          className={`px-2.5 py-1 border rounded transition font-mono cursor-pointer ${editor.isActive('codeBlock') ? 'bg-black text-white' : 'bg-gray-50 hover:bg-gray-100'}`}
        >
          Code Block
        </button>
        <span className="w-px h-4 bg-gray-300 mx-1"></span>
        ...
        <button
          type="button"
          onClick={setLink}
          className={`px-2.5 py-1 border rounded transition cursor-pointer ${editor.isActive('link') ? 'bg-black text-white' : 'bg-gray-50 hover:bg-gray-100'}`}
        >
          Link
        </button>
