...
export default function TeamsPage() {
  ...
  // Member addition is done using registered user email.
  ...
  useEffect(() => {
    const initWorkspaceAndToken = async () => {
      ...
      try {
        ...
        setError(
          'Could not retrieve your workspace. Please check your connection or re-login.'
        );
      }
    };
  ...
  const fetchTeams = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/teams?workspace_id=${workspaceId}`
      );
      ...
      setError(
        typeof errMsg === 'string'
          ? errMsg
          : 'Failed to load teams from server.'
      );
    }
  };
  ...
  // 6. Handle adding a member using Email
  const handleAddMember = async (teamId: string) => {
    ...
    try {
      ...
      // Keep managing section open so the error is visible.
    }
  };
        ...
        {teams.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 items-start">
            {teams.map((team) => (
              <div
                key={team.id}
                className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 flex flex-col justify-between"
              >
                <div>
                      ...
                      <div className="mb-3 p-3 bg-gray-50 border border-gray-200 rounded-xl flex flex-col gap-2.5 text-xs">
                        <div>
                          ...
                          <p className="text-[10px] text-gray-500 mt-1">
                            Note: Enter the registered email of the
                            user.
                          </p>
                        </div>
                    ...
                    <div className="max-h-28 overflow-y-auto space-y-1.5">
                      ...
                      team.team_members.length > 0 ? (
                        team.team_members.map((member) => (
                          <div
                            key={member.id}
                            className="flex items-center justify-between text-[11px] bg-white p-1.5 rounded-lg border"
                          >
                            <span
                        ...
                        <p className="text-[11px] text-[var(--muted-light)] italic">
