...
async def autosave_document(
    ...
    # After successful autosave, create a new version
    doc = result.get("data")  # Assuming result contains the updated document
    if doc:
        await DocumentVersionService.create_version(
            session=db,
            document_id=document_id,
            created_by=doc.owner_id,  # or use the authenticated user's ID
            content=doc.content,
        )
    return result
